...
public partial class Basic : Page
{
    ...
    private async void LoadWeatherData()
    {
        ...
        {
            log.Info($"[WEATHER PARSER]: Failed to load weather data for city '{city}'. City not found or not set.");
        }
    }
    ...
    private async Task GetWeatherAsync(string city)
    {
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                string cloudiness = weatherData["clouds"]?["all"]?.ToString();
                double wind = Double.Parse(weatherData["wind"]?["speed"]?.ToString());
                string cityResult = weatherData["name"]?.ToString();

                UpdateWeatherUI(description, temperature, humidity, cloudiness, wind, cityResult);
            }
            else
            {
                Console.WriteLine("Не удалось получить данные о погоде. Проверьте правильность названия города.");
            }
        }
        catch (Exception ex)
        {
    ...
    private void UpdateWeatherUI(string description, double temperature, string humidity, string cloudiness, double wind, string cityResult)
    {
        WeatherBlock.Text = description;
        TempBlock.Text = $"{Math.Round(temperature)}°C";
        HumidityBlock.Text = humidity + "%";
        CloudinessBlock.Text = cloudiness + "%";
        WindBlock.Text = Math.Round(wind) + " м/с";
        CityBlock.Text = cityResult;
    }
